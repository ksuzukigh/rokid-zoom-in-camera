...
[スマートフォンで最新版APKをダウンロード](https://github.com/ksuzukigh/rokid-zoom-in-camera/releases/latest/download/Rokid-ZOOM-IN-CAMERA.apk)
...
> Rokid本体の単色表示に近づけた再現画像です。実際の見え方とは異なります。
...
- 動画撮影中はRV101の電子式手ぶれ補正（EIS）を自動で使用
- 倍率、電池残量、録画時間を画面内に表示
...
ZOOM IN CAMERAは、動画撮影時にRokid本体の電子式手ぶれ補正（EIS）機能を使っています。

このアプリの特徴はズーム撮影ですが、ズーム時の動画撮影では、手ぶれ補正の効果に限界があるようです。

撮影した動画を分析・計測したところ、ONはOFFより横方向で約9％、縦方向で約20％小さく出たというデータもあります。しかし、体感で「効果あり」といえるほどの実力はないかもしれません。

市販のビデオカメラの実力には到底及ばないため、ご了承ください。
...
撮影直後の写真や動画が一覧に出ないときは、Hi Rokidアプリ自体をいったん閉じて、開き直してください。

1.2.1以降では、写真は`DCIM/Camera`、動画はHi Rokidが読み取る`Movies/Camera`へ保存します。1.2.0から上書き更新した場合、Rokidに残っている本アプリの動画は、更新後の初回起動時にHi Rokid用のコピーを自動で作ります。元の動画は削除しません。
...
スマートフォンから入れられない場合の予備手段です。MacまたはWindows 10/11のPC、Rokidの開発用5ピンケーブル、Hi Rokidで有効にした開発者モード（ADB）が必要です。RV101付属の3ピンケーブルは充電専用で、アプリのインストールには使えません。
...
3. 「実行」を押します。
4. Windows画面にADBの許可確認が表示された場合は許可します。
