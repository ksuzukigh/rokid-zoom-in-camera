...
写真はRV101が出力できる最大のJPEGサイズを自動で選びます。動画は1920×1080・30fpsを明示的に指定します。Hi Rokid側の設定を変更しても、Rokid本体の`DCIM/Camera`へ保存する元ファイルの解像度とフレームレートは原則として変わりません。Hi Rokidへ取り込んだ後の表示や圧縮、透かしの有無は別の処理です。

倍率を上げても保存ファイルのピクセル数は同じです。ズームは中央を切り出して拡大するため、倍率が高いほど細部は粗くなります。
